...
class AnalysisSample:
    ...
    def run_batch_jobs(self,inputfiles,runmode):
        ...
        if ( self.sampleDataSet.find("DYJetsToLL_M-50")>=0 ):
            ...
            # Less jobs for QCD
            numJobs=min(25,(len(inputfiles)/5)+1)
            pass
...
#            tmp_dir+="/"+self.workdir.replace("/","_")
            ...
            if runmode=="FNAL":
                jobscript.write("cd -\n")
                jobscript.write("cmsRun job"+str(i+1)+"_cfg.py\n")
            else:
            ...
            if runmode=="CERN":
                ...
                condorscript=open(self.workdir+"/condor"+str(i+1),"w")
                condorscript.write("universe = vanilla\n")
                condorscript.write("Executable = job"+str(i+1)+".sh\n")
                condorscript.write('Requirements = Memory >= 199 &&OpSys == "LINUX"&& (Arch != "DUMMY" )&& Disk > 1000000\n')
                condorscript.write("Should_Transfer_Files = YES\n")
                condorscript.write("WhenToTransferOutput = ON_EXIT\n")
                condorscript.write("Transfer_Input_Files = job"+str(i+1)+".sh, job"+str(i+1)+"_cfg.py\n")
                condorscript.write("Output = job_$(Cluster)_$(Process).stdout\n")
                condorscript.write("Error = job_$(Cluster)_$(Process).stderr\n")
                condorscript.write("Log = job_$(Cluster)_$(Process).log\n")
                # condorscript.write("notify_user = @FNAL.GOV")
                # condorscript.write("Arguments = 60\n")
...
#        trigger="HLT_DoublePhoton48_HEVT_v"
        trigger="HLT_Photon36_R9Id85_OR_CaloId10_Iso50_Photon22_R9Id85_OR_CaloId10_Iso50_v"
        pass
...
def runPileUpCalc(workdir,lumiJson,puJson,output,minBiasXSec):
    if workdir=="":
    ...
    if output=="":
        ...
        pass
    if puJson=="":
        print "ERROR: No pu json file provided for pileupCalc.py"
        sys.exit(1)
        pass
    
    # Get pileup json file
    print "Getting pileup json file"
    os.system("cd "+workdir+"; cp "+puJson+" pileupJson.txt")
    
    # Run pileup calc
    print "Running pileupCalc.py"
    print "With minbias cross section %.4g pb" % minBiasXSec
    print "And pu json file :",puJson.split("/")[-1]
    os.system("pileupCalc.py -i "+lumiJson+" --inputLumiJSON "+workdir+"/pileupJson.txt --calcMode true --minBiasXsec "+str(minBiasXSec)+"  --maxPileupBin 60 --numPileupBins 300 "+output)

    pass

def getPUInfo(workdir,analysisSample):
    
    # PU Json file depends on run range (at the moment.  Files with pixel corrections not yet available)
    puJson=""
    ...
    # Run pileupCalc.py
    minBiasXSec=69300.
    runPileUpCalc(workdir,workdir+"/crabDir/res/lumiSummary.json ",puJson,workdir+"/pileup.root",minBiasXSec)
    # For systematic studies
    # Varies minbias xsec by +/-5%
    runPileUpCalc(workdir,workdir+"/crabDir/res/lumiSummary.json ",puJson,workdir+"/pileup_p5.root",minBiasXSec*1.05)
    runPileUpCalc(workdir,workdir+"/crabDir/res/lumiSummary.json ",puJson,workdir+"/pileup_m5.root",minBiasXSec*0.95)
    ...
    # Just write over pileup files for systematics studies
    pileupFile=analysisMacroDir+"pileup_p5_"+analysisSample.id+".root"
    os.system("cp "+workdir+"/pileup_p5.root "+pileupFile)
    pileupFile=analysisMacroDir+"pileup_m5_"+analysisSample.id+".root"
    os.system("cp "+workdir+"/pileup_m5.root "+pileupFile)
    pass
