...
class DileptonAnalyzer : public treeAnalyzerBase {
  public :
    ...
    static const TreeLepton getLepton( const std::vector<TreeLepton> & leptons, unsigned int index );
    //DeltaPhi function
    static double DeltaPhi(double phi1, double phi2);
    //DeltaR calculator
    static double DeltaR(const TreeLepton & lepton1, const TreeLepton & lepton2);
  ...
  private :
    ...
    // void studyTrackingEfficiency(const std::vector<TreeLepton> & single_cosmic_muons);
    void studyTrackingEfficiency( const std::vector<TreeLepton> & single_cosmic_muons, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates);

    // Check if signal trigger fired (used if signal MC)
    bool signalTriggerFired(const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates);
    ...
    unsigned int storeNumberOfExoticsALL(const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates);

    // Store number of exotics that have been RECO'ed
    void storeNumberOfExoticsRECO( TreeDipseudoLeptonCandidate & cand, TreeLepton &leptonL, TreeLepton &leptonH, ofstream & oneExoFile, ofstream & twoExoFile, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates );
    ...
    void studyCandidate( std::vector <TreeLepton> single_cosmic_muons, TreeDipseudoLeptonCandidate &cand, TreeLepton& leptonL, TreeLepton& leptonH , Histograms &h, bool fillTextFiles, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates );
...
//  new function to list the single cosmic muons

    void singleCosmicMuons( const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates, std::vector<TreeLepton> & single_cosmic_muon );
    ...
    void cleaningCandidates(std::vector<TreeDipseudoLeptonCandidate> & cleanCandidates);
...
//    void findGenMatch( TreeLepton &lepton );
    ...
    DileptonAnalysisCuts dileptonCuts_finalColl_muTrack;
    ...
    double nTotalCands_;
    double nTotalCands_correctly1;
    double nTotalCands_final1;
    double nTotalCands_correctly2;
    double nTotalCands_final2;
    double nTotalCands_Data;
    double nTotalCands_Data_control_TempX;
    double nTotalCands_Data_control;
};
