...
void DileptonAnalyzer::studyTrackingEfficiency(const std::vector<TreeLepton> & single_cosmic_muons, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
  if( nSensitiveExoticDecayInEvent_ != 1 ) return;

  if ( !isCentralCtau() )  return;
  ...
  if (fabs(candidates->ll1_daughterPdgId) == interestingLepton) hists_.h_all_lxy->Fill( candidates->ll1_decayLength2D);
  if (fabs(candidates->ll2_daughterPdgId) == interestingLepton) hists_.h_all_lxy->Fill( candidates->ll2_decayLength2D);

  if ( fabs(candidates->ll1_daughterPdgId) == interestingLepton && ll1WithinAcceptance() && isCentralCtau() && fabs(candidates->ll1_daughter1_Z0)<50 && fabs(candidates->ll1_daughter2_Z0)<50)
    hists_.h_all_d0_2D->Fill( fabs(candidates->ll1_daughter1_D0), fabs(candidates->ll1_daughter2_D0) );
  if ( fabs(candidates->ll2_daughterPdgId) == interestingLepton && ll2WithinAcceptance() && isCentralCtau() && fabs(candidates->ll2_daughter1_Z0)<50 && fabs(candidates->ll2_daughter2_Z0)<50)
    hists_.h_all_d0_2D->Fill( fabs(candidates->ll2_daughter1_D0), fabs(candidates->ll2_daughter2_D0) );
    ...
    if ( fabs(candidates->ll1_daughterPdgId) == interestingLepton ) {
      ...
      if ( candidates->ll1_daughter1_Pt >= acceptancePt1Cut_ && fabs(candidates->ll1_daughter1_Eta) < acceptanceEtaCut_ &&
	   ...
	   candidates->ll1_decayLength2D <= acceptanceLxyCut_ ){
        ...
        if ( signalTriggerFired(cleanedCandidates) ) {
  ...
  for( unsigned int iTreeCand=0; iTreeCand != cleanedCandidates.size(); ++iTreeCand ) {
    ...
    if ( !signalTriggerFired(cleanedCandidates) ) continue;
  ...
  for( unsigned int iTreeCand=0; iTreeCand != cleanedCandidates.size(); ++iTreeCand ) {
    ...
    if ( !signalTriggerFired(cleanedCandidates) ) continue;
    if ( !correctLeptonsInCandidate(cand, leptonL, leptonH ) ) continue;
    hists_.h_reco_lxy->Fill(cand.decayLength_PV);
    ...
    bool plotLeptonH=false;
    for ( unsigned int iPdg = 0; iPdg < signalPdgId_.size(); iPdg++ ) {
      if ( leptonH.genSignalOriginPdgId == signalPdgId_[iPdg] ) {
        plotLeptonH = true;
        break;
      }
    }
    if (!plotLeptonL || !plotLeptonH) continue;
    */

    //Define final coll cuts here
    DileptonAnalysisCuts::PassedWhichCuts finalCollCuts_studyTrack = dileptonCuts_finalColl_.whichCuts( single_cosmic_muons, candidates, triggers,
    ...
    finalCollCutsTemp_studyTrack.passLeptonD0 = true;
    //finalCollCutsTemp_studyTrack.passIso = true;
    //finalCollCutsTemp_studyTrack.passDeltaPhi = true;
    //finalCollCutsTemp_studyTrack.passMinLxySig = true;
    //finalCollCutsTemp_studyTrack.passMinMuonValidStations = true;
    //finalCollCutsTemp_studyTrack.passEta = true;
    //finalCollCutsTemp_studyTrack.passVertexChi2 = true;
    //finalCollCutsTemp_studyTrack.passDeltaR = true;
    //finalCollCutsTemp_studyTrack.passCosine = true;
    ...
    float cand_reco_LxySig = fabs(cand.decayLengthSignificance_PV);
    hists_.h_reco_Lxy_gen_Lxy->Fill(cand_reco_Lxy, fabs(leptonL.genSignalOriginLxy));
    hists_.h_reco_LxySig_gen_Lxy->Fill(cand_reco_LxySig, fabs(leptonL.genSignalOriginLxy));
    hists_.h_reco_LxyError_gen_Lxy->Fill(cand_reco_Lxy/cand_reco_LxySig, fabs(leptonL.genSignalOriginLxy));

    hists_.h_reco_d0->Fill(leptonL.genD0);
    hists_.h_reco_d0->Fill(leptonH.genD0);
    hists_.h_reco_d0_gen_d0->Fill(fabs(leptonL.d0_PV), fabs(leptonL.genD0));
    hists_.h_reco_d0_gen_d0->Fill(fabs(leptonH.d0_PV), fabs(leptonH.genD0));
    hists_.h_reco_d0Sig_gen_d0->Fill(fabs(leptonL.d0Significance_PV_includingPVError), fabs(leptonL.genD0));
    hists_.h_reco_d0Sig_gen_d0->Fill(fabs(leptonH.d0Significance_PV_includingPVError), fabs(leptonH.genD0));
    hists_.h_reco_d0Error_gen_d0->Fill(fabs(leptonL.d0_PV)/fabs(leptonL.d0Significance_PV_includingPVError), fabs(leptonL.genD0));
    hists_.h_reco_d0Error_gen_d0->Fill(fabs(leptonH.d0_PV)/fabs(leptonH.d0Significance_PV_includingPVError), fabs(leptonH.genD0));
    if ( dileptonCuts_finalColl_.passAllCuts( finalCollCutsTemp_noCosmicRejection ) ) {
      hists_.h_reco_lxy_aftercuts_noCosmicRejection->Fill(leptonL.genSignalOriginLxy);
      if ( dileptonCuts_finalColl_.passAllCuts( finalCollCutsTemp_studyTrack ) ) {
        hists_.h_reco_lxy_aftercuts->Fill(leptonL.genSignalOriginLxy);
        hists_.h_reco_Lxy_gen_Lxy_aftercuts->Fill(cand_reco_Lxy, fabs(leptonL.genSignalOriginLxy));
        hists_.h_reco_LxySig_gen_Lxy_aftercuts->Fill(cand_reco_LxySig, fabs(leptonL.genSignalOriginLxy));
        hists_.h_reco_LxyError_gen_Lxy_aftercuts->Fill(cand_reco_Lxy/cand_reco_LxySig, fabs(leptonL.genSignalOriginLxy));
        ...
        if( DileptonAnalysisCuts::isComplementary(leptonL, leptonH, candidates, triggers, dileptonCuts_finalColl_muTrack) ) {
...
bool DileptonAnalyzer::signalTriggerFired(const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
  for ( unsigned int iTrig=0; iTrig<triggers->size(); iTrig++ ) {
	// Loop over signal trigger to see if one has fired
    //if ( triggers->at(iTrig) == signalTrigger_ ) return true;
    if ( triggers->at(iTrig) == "HLT_L2DoubleMu23_NoVertex_2Cha_Angle2p5_v2" ) {
      //for( unsigned int iTreeCand=0; iTreeCand != cleanedCandidates.size(); ++iTreeCand ) {
      //  TreeDipseudoLeptonCandidate cand = cleanedCandidates.at(iTreeCand);
      //  if ( cand.deltaR>0.2 ) return true;
      //} 
	  return true;
    }
    //if ( triggers->at(iTrig) == "HLT_MET120_HBHENoiseCleaned_v3" ) {
    //  for( unsigned int iTreeCand=0; iTreeCand != cleanedCandidates.size(); ++iTreeCand ) {
    //    TreeDipseudoLeptonCandidate cand = cleanedCandidates.at(iTreeCand);
    //    if ( cand.ptCorr>200 ) return true;
    //  }
	//  //return true;
    //}
  }
...
const TreeLepton DileptonAnalyzer::getLepton( const std::vector<TreeLepton> & leptons, unsigned int index )
{
  // Loop over TreeLeptons
  for ( unsigned int i = 0; i < leptons.size(); i++ ) {
    if ( leptons[i].index >=0 ) {
      if ( index == abs( leptons[i].index ) ) return leptons[i];
    }
  }
...
unsigned int DileptonAnalyzer::storeNumberOfExoticsALL(const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
  ...
  bool sigTrig=false;
  if (signalTriggerFired(cleanedCandidates)) sigTrig=true;
...
void DileptonAnalyzer::storeNumberOfExoticsRECO( TreeDipseudoLeptonCandidate & cand, TreeLepton &leptonL,
                                                 TreeLepton &leptonH, ofstream & oneExoFile, ofstream & twoExoFile, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
...
void DileptonAnalyzer::monitorCutFlow( DileptonAnalysisCuts::PassedWhichCuts & whichCuts, map< TString, float > & cutFlowMap, double weight )
{
  ...
  if ( whichCuts.passMinMuonValidHits ) cutFlowMap["MinMuonValidHits"] += weight; else return;
...
{
  // Store in a TProfile
  TH1F *cutFlow = new TH1F("cutFlow","Cut flow for final set of analysis cuts",18,0.5,18.5);
  ...
  // Store in a TProfile
  TH1F *cutFlowColl = new TH1F("cutFlowColl","Cut flow for final set of analysis cuts",19,0.5,19.5);
  cutFlowAxis = cutFlowColl->GetXaxis();

  // Store number of signal MC exotics before cut flow in separate histogram
  if ( isSignalMC_ ) {
    TH1F *cutFlowColl_preselection = new TH1F("cutFlowColl_preselection","Cut flow for preselection of analysis cuts",18,0.5,18.5);
...
void DileptonAnalyzer::studyCollinearity( std::vector <TreeLepton> single_cosmic_muons, TreeDipseudoLeptonCandidate &cand, TreeLepton & leptonL, TreeLepton & leptonH,
                                          double & mass,Histograms & h, bool fillCorrectedTextFiles, const DileptonAnalysisCuts & dileptonCuts_finalColl_muTrack, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
  ...
  // WARNING: This is not changed to the single_cosmic_muons
  DileptonAnalysisCuts::PassedWhichCuts finalCollCuts = dileptonCuts_finalColl_.whichCuts( single_cosmic_muons, candidates, triggers, 
											   cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_,
											   &dileptonCuts_finalColl_muTrack);
  DileptonAnalysisCuts::PassedWhichCuts looseCollCuts = dileptonCuts_looseColl_.whichCuts( single_cosmic_muons, candidates, triggers,
											   cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_,
											   &dileptonCuts_finalColl_muTrack);
  DileptonAnalysisCuts::PassedWhichCuts controlCollCuts = dileptonCuts_controlColl_.whichCuts( single_cosmic_muons, candidates, triggers,
											       cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_,
											       &dileptonCuts_finalColl_muTrack);
  DileptonAnalysisCuts::PassedWhichCuts looseControlCollCuts = dileptonCuts_looseControlColl_.whichCuts( single_cosmic_muons, candidates, triggers,
													 cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_,
													 &dileptonCuts_finalColl_muTrack);
  ...
  if ( isSignalMC_ ) {
    if ( dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) {
      storeNumberOfExoticsRECO( cand, leptonL, leptonH, recoExotics_coll_oneSensitiveDecay_, recoExotics_coll_twoSensitiveDecay_, cleanedCandidates );
      ...
      if ( isCentralCtau() && nSensitiveExoticDecayInEvent_==1 && eventWithinAcceptance( nSensitiveExoticDecayInEvent_ ) ) {
        ...
        if ( ( interestingLepton == fabs(candidates->ll1_daughter1_PdgId) && interestingLepton == fabs(candidates->ll1_daughter2_PdgId) ) ) {
          ...
          if ( candidates->ll1_decayLength2D > 0 && candidates->ll1_decayLength2D < acceptanceLxyCut_ ) h.h_recoExotics_coll_one_XPt->Fill(candidates->ll1_pt, eventWeight_);
        }
          ...
          if ( candidates->ll2_decayLength2D > 0 && candidates->ll2_decayLength2D < acceptanceLxyCut_ ) h.h_recoExotics_coll_one_XPt->Fill(candidates->ll2_pt, eventWeight_);
        }
      ...
      storeNumberOfExoticsRECO( cand, leptonL, leptonH, recoExotics_coll_oneSensitiveDecay_negative_, recoExotics_coll_twoSensitiveDecay_negative_, cleanedCandidates );
    }
  }
 ...
 //     std::cout << "minDRL = " << leptonL.minMatchedDeltaR_1 << " minDRH = " << leptonH.minMatchedDeltaR_1 << std::endl;
  ...
  if ( isSignalMC_ && isCentralCtau() ) {
   ...
   if ( dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) {
    ...
    if ( nSensitiveExoticDecayInEvent_ == 1 ) {
      ...
      if ( dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) nTotalCands_final1++;
    }
    if ( nSensitiveExoticDecayInEvent_ == 2 ) {
      if ( correctlyReconstructed( cand, leptonL, leptonH ) ) nTotalCands_correctly2++;
      if ( dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) nTotalCands_final2++;
    }
  }
  ...
  fillPlots( cand, leptonL, leptonH, mass, dileptonCuts_controlColl_, controlCollCuts, h.histMapControlColl, false );
...
*/
  ...
  finalCollCuts_removedLifetime.passMaxLxySig = true;
  finalCollCuts_removedLifetime.passMinLxySig = true;
...
void DileptonAnalyzer::studyCandidate( std::vector <TreeLepton> single_cosmic_muons, TreeDipseudoLeptonCandidate &cand, TreeLepton& leptonL, TreeLepton& leptonH , Histograms & h, bool fillCorrectedTextFiles, const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates)
{
  ...
  //
  studyCollinearity( single_cosmic_muons, cand, leptonL, leptonH, mass, h, fillCorrectedTextFiles, dileptonCuts_finalColl_muTrack, cleanedCandidates);
                                  ...
                                  map< TString, TH1* > & histMap, bool isFinalCuts, bool debug )
{
  ...
  if ( cuts.passAllCuts( cutsTemp ) ) {
    histMap["maxTrackChi2"]->Fill( std::max(leptonL.trackChi2, leptonH.trackChi2), eventWeight_);
  }
  ...
  // SA Muon Number of DT or CSC Valid Hits
  cutsTemp = passedWhichCuts;
  cutsTemp.passMinMuonValidHits = true;

  if (  cuts.passAllCuts( cutsTemp )  ) {
    histMap["minValidHits"]->Fill( min(leptonH.numberOfValidMuonHits, leptonL.numberOfValidMuonHits), eventWeight_ );
    histMap["minValidDTHits"]->Fill( min(leptonH.numberOfValidMuonDTHits, leptonL.numberOfValidMuonDTHits), eventWeight_ );
    histMap["minValidCSCHits"]->Fill( min(leptonH.numberOfValidMuonCSCHits, leptonL.numberOfValidMuonCSCHits), eventWeight_ );
  }
                                                 ...
                                                 const bool passVertex, Histograms &h )
{
  ...
  if ( passVertex ) {
    ...
    if ( fabs(cand.leptonD0SignificanceL_PVrefit_includingPVError) > 12 )
      h.h_leptonD0SigFractionMap_pass->Fill( 2 * atan( exp(-leptonL.eta)), leptonL.phi, eventWeight_ );
    if ( fabs(cand.leptonD0SignificanceH_PVrefit_includingPVError) > 12 )
...
void DileptonAnalyzer::singleCosmicMuons( const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates, std::vector<TreeLepton> & single_cosmic_muon ) 
{
  // int numLeptons = 0;
  for ( unsigned int iTreeLepton=0; iTreeLepton != candidates->leptons_.size(); ++iTreeLepton ) {
    TreeLepton lepton = candidates->leptons_.at(iTreeLepton);
...
//       TreeDipseudoLeptonCandidate cand = candidates->candidates_.at(iTreeCand);
    for( unsigned int iTreeCand=0; iTreeCand != cleanedCandidates.size(); ++iTreeCand ) {
      ...
      TreeLepton leptonL = getLepton (candidates->leptons_, cand.leptonIndexL);
      TreeLepton leptonH = getLepton (candidates->leptons_, cand.leptonIndexH);
      if( !(leptonL.isStandAloneMuon && leptonH.isStandAloneMuon) ) continue;
    ...
    if (use_this_lepton) {
      // std::cout << "found a single lepton" << std::endl;
      single_cosmic_muon.push_back(lepton);
    }
  }
  // std::cout << "total number of RSA = " << numLeptons << std::endl;
}
...
//      singleCosmicMuons(candidates->candidates_, single_cosmic_muons);
    ...
    if ( isSignalMC_ ) {
      ...
      nSensitiveExoticDecayInEvent_=storeNumberOfExoticsALL(cleanedCandidates);
      ...
      if ( isCentralCtau() ){
        ...
        if ( nSensitiveExoticDecayInEvent_==2 ) {
          ...
          nExoticsBeforeAcceptance_ += eventWeight_;
      ...
      studyTrackingEfficiency(single_cosmic_muons, cleanedCandidates);
...
//        k++;
	...
	if ( !isDitauEvent() || isData_ || isSignalMC_ ) {
	  studyCandidate( single_cosmic_muons, cand, leptonL, leptonH, hists_, false, cleanedCandidates);
	}
	else {
	  studyCandidate( single_cosmic_muons, cand, leptonL, leptonH, histsTau_, false, cleanedCandidates);
	}
  ...
  nTotalCands_Data_control_TempX(0), nTotalCands_Data_control(0) 
{
  ...
  // Is this data?
  if ( fileName.Contains("Data") || fileName.Contains("cosmic") ) isData_=true;
...
//  int position=fileLabel.Index("/uscms_data/d3/msolmaz/myworkingspace/phys_698_spring_2014/CMSSW_5_3_8/src/workdirs/")+84;
  ...
  if ( fileName.Contains("LongLived") || fileName.Contains("Chi0ToNuLL") ) {
    ...
    if ( anaType_==_2eTrack ) {
      ...
      //signalTriggers_.push_back("HLT_PFMET150_v4");
      signalTriggers_.push_back("HLT_MET120_HBHENoiseCleaned_v3");      
      triggerToPlot_=1;
    }
  ...
  for (unsigned int i = 0; i < cleanCandidates.size(); i++) {
    for (unsigned int j = i+1; j < cleanCandidates.size(); j++) {
      if (cleanCandidates.at(i).leptonIndexL == cleanCandidates.at(j).leptonIndexL ||
          cleanCandidates.at(i).leptonIndexL == cleanCandidates.at(j).leptonIndexH ||
          cleanCandidates.at(i).leptonIndexH == cleanCandidates.at(j).leptonIndexL ||
          cleanCandidates.at(i).leptonIndexH == cleanCandidates.at(j).leptonIndexH) {
        cleanCandidates.erase(cleanCandidates.begin()+j);
        j--;
      }
    }
  }
