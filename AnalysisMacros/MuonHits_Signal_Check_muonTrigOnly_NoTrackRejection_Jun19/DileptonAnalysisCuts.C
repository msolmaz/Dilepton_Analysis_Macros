...
minDeltaR_(-1), minPt_MET_(-9999), minCosine_(-9999), minDeltaPhi_(0), maxDeltaPhi_(99999999999), minMass_(0), min_csc_dt_station_valid_hits_(0), use_PV(true), cosinecut_cosmics(-999.0), track_reject_DR(9999.0) 
{
...
DileptonAnalysisCuts::PassedWhichCuts DileptonAnalysisCuts::whichCuts( const std::vector<TreeLepton> & single_cosmic_leptons, 
								       const Candidates * candidates, const std::vector<std::string> * triggers, 
								       ...
								       const DileptonAnalysisCuts * dileptonCuts_finalColl_muTrack) const
{
  ...
  for ( unsigned int iTrig=0; iTrig<triggers->size(); iTrig++ ) {
    ...
    if ( triggers->at(iTrig).find("HLT_L2DoubleMu23_NoVertex_2Cha_Angle2p5") != std::string::npos ) DiMuTrigFired = true;
    //if ( triggers->at(iTrig).find("HLT_MET120_HBHENoiseCleaned") != std::string::npos ) MetTrigFired = true;
    //if ( triggers->at(iTrig).find("HLT_L2DoubleMu") != std::string::npos ) DiMuTrigFired = true;
    //if ( triggers->at(iTrig).find("HLT_MET120_v") != std::string::npos ) MetTrigFired = true;
  }
  if ( ( DiMuTrigFired && candidate.deltaR>minDeltaR_ ) || ( MetTrigFired && candidate.ptCorr>minPt_MET_ ) || analysis == _2eTrack ) passedCuts.passDeltaR=true;
...
/*  if (analysis == _2saMu) {
    if (std::min(leptonH.numberOfValidMuonCSCHits, leptonL.numberOfValidMuonCSCHits) >= min_csc_valid_hits_ ||
        std::min(leptonH.numberOfValidMuonDTHits, leptonL.numberOfValidMuonDTHits) >= min_dt_valid_hits_ ) passedCuts.passMinMuonValidHits=true;
  }
  else passedCuts.passMinMuonValidHits=true;
*/
...
void DileptonAnalysisCuts::printCutValues() {
  ...
  cout << "Minimum dilepton pt used for MET trigger : " << minPt_MET_ << endl;
...
bool DileptonAnalysisCuts::isComplementaryExtrapolated(const TreeDipseudoLeptonCandidate & cand, const Candidates * candidates, const std::vector<std::string> * triggers, 
						       const DileptonAnalysisCuts & dileptonCuts_finalColl_muTrack )
{
  ...
  for( ; it != cand.tkMatches.end(); ++it ) {
    ...
    if(trackL.isCentralTrack && trackH.isCentralTrack) {
      DileptonAnalysisCuts::PassedWhichCuts finalCollCuts_muTrack = dileptonCuts_finalColl_muTrack.whichCuts( candidates->leptons_, candidates, triggers, 
...
bool DileptonAnalysisCuts::isComplementary(const TreeLepton & leptonL, const TreeLepton & leptonH,
					   const Candidates * candidates, const std::vector<std::string> * triggers, const DileptonAnalysisCuts & dileptonCuts_finalColl_muTrack )
{
