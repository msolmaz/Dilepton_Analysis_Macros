...
void DileptonAnalyzer::studyTrackingEfficiency()
{
  ...
  if ( genLeptonPt.size() == 0 ) return;
   ...
   double cand_match_mass = -9999999999.0;
...
//      if ( lepton.triggerMatch == 0 ) plotLepton = true;
      if ( lepton.triggerMatch == 0 ) continue;

//Find the candidate that the lepton is associated with....
         double cand_reco_Lxy, cand_reco_LxySig;
	   ...
	   if ( !correctLeptonsInCandidate(cand, leptonL, leptonH ) ) continue;
           ...
           DileptonAnalysisCuts::PassedWhichCuts finalCollCuts_studyTrack = dileptonCuts_finalColl_.whichCuts( candidates->leptons_, cand, cand.mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
           ...
           finalCollCutsTemp_studyTrack.passRejectionCosmics = true;
           finalCollCutsTemp_studyTrack.passMinMuonValidStations = true;
           if ( (leptonL.genPt == lepton.genPt || leptonH.genPt == lepton.genPt) && plotLepton ) {
        int exo=-1;
        for ( unsigned int iExo = 0; iExo < genLeptonPt.size(); iExo++ ) {
          if ( lepton.genPt == genLeptonPt[iExo] ) {
            ...
            if ( genLeptonPt[iExo]>=21 && fabs( genLeptonEta[iExo] ) < 2 ) {
              ...
              hists_.h_reco_d0->Fill( lepton.genPt);
              hists_.h_reco_lxy->Fill(lepton.genPt);
              hists_.h_reco_d0_gen_d0->Fill(fabs(lepton.d0_PV), fabs(genLeptonD0[exo]));
              hists_.h_reco_d0Sig_gen_d0->Fill(fabs(lepton.d0Significance_PV_includingPVError), fabs(genLeptonD0[exo]));
              hists_.h_reco_d0Error_gen_d0->Fill(fabs(lepton.d0_PV)/fabs(lepton.d0Significance_PV_includingPVError), fabs(genLeptonD0[exo]));
              if ( dileptonCuts_finalColl_.passAllCuts( finalCollCutsTemp_studyTrack ) ) {
              hists_.h_reco_d0_gen_d0_aftercuts->Fill(fabs(lepton.d0_PV), fabs(genLeptonD0[exo]));
              hists_.h_reco_d0Sig_gen_d0_aftercuts->Fill(fabs(lepton.d0Significance_PV_includingPVError), fabs(genLeptonD0[exo]));
              hists_.h_reco_d0Error_gen_d0_aftercuts->Fill(fabs(lepton.d0_PV)/fabs(lepton.d0Significance_PV_includingPVError), fabs(genLeptonD0[exo]));
            ...
            break;
              ...
              if ( cand_match_mass != cand.mass ){
              cand_reco_Lxy = fabs(cand.decayLength_PV);
              cand_reco_LxySig = fabs(cand.decayLengthSignificance_PV);
              hists_.h_reco_Lxy_gen_Lxy->Fill(cand_reco_Lxy, fabs(lepton.genSignalOriginLxy));
              hists_.h_reco_LxySig_gen_Lxy->Fill(cand_reco_LxySig, fabs(lepton.genSignalOriginLxy));
              hists_.h_reco_LxyError_gen_Lxy->Fill(cand_reco_Lxy/cand_reco_LxySig, fabs(lepton.genSignalOriginLxy));
              if ( dileptonCuts_finalColl_.passAllCuts( finalCollCutsTemp_studyTrack ) ) {
              hists_.h_reco_Lxy_gen_Lxy_aftercuts->Fill(cand_reco_Lxy, fabs(lepton.genSignalOriginLxy));
              hists_.h_reco_LxySig_gen_Lxy_aftercuts->Fill(cand_reco_LxySig, fabs(lepton.genSignalOriginLxy));
              hists_.h_reco_LxyError_gen_Lxy_aftercuts->Fill(cand_reco_Lxy/cand_reco_LxySig, fabs(lepton.genSignalOriginLxy));
                    }
              cand_match_mass = cand.mass;
              break;
               }
            }
...
void DileptonAnalyzer::reportCutFlow( TFile & outputFile )
{
  ...
  // Store in a TProfile
  TH1F *cutFlowColl = new TH1F("cutFlowColl","Cut flow for final set of analysis cuts",18,0.5,18.5);
                                       ...
                                       double & mass, Histograms & h, bool fillTextFiles )
{
  ...
  DileptonAnalysisCuts::PassedWhichCuts finalCuts = dileptonCuts_final_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
  DileptonAnalysisCuts::PassedWhichCuts removedLifetimeCuts = dileptonCuts_removedLifetime_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
                                          ...
                                          double & mass,Histograms & h, bool fillCorrectedTextFiles )
{
  ...
  DileptonAnalysisCuts::PassedWhichCuts finalCollCuts = dileptonCuts_finalColl_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
  DileptonAnalysisCuts::PassedWhichCuts looseCollCuts = dileptonCuts_looseColl_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
  DileptonAnalysisCuts::PassedWhichCuts controlCollCuts = dileptonCuts_controlColl_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
  DileptonAnalysisCuts::PassedWhichCuts looseControlCollCuts = dileptonCuts_looseControlColl_.whichCuts( candidates->leptons_, cand, mass, leptonL, leptonH, anaType_, lxyScale_, d0Scale_ );
    ...
    if ( dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) {
      ...
      if ( isCentralCtau() && nSensitiveExoticDecayInEvent_==1 && eventWithinAcceptance( nSensitiveExoticDecayInEvent_ ) ) {
        ...
        if ( ( interestingLepton == fabs(candidates->ll1_daughter1_PdgId) && interestingLepton == fabs(candidates->ll1_daughter2_PdgId) ) ) {
          ...
          if ( candidates->ll1_decayLength2D > 0 && candidates->ll1_decayLength2D < 500 ) h.h_recoExotics_coll_one_XPt->Fill(candidates->ll1_pt, eventWeight_);
        }
          ...
          if ( candidates->ll2_decayLength2D > 0 && candidates->ll2_decayLength2D < 500 ) h.h_recoExotics_coll_one_XPt->Fill(candidates->ll2_pt, eventWeight_);
        }
  ...
  if ( isData_ ) {
    ...
    if ( unblind_ && dileptonCuts_finalColl_.passAllCuts( finalCollCuts ) ) {
      ...
      // std::cout << "pt = " << leptonL.pt << std::endl;
      // std::cout << "eta = " << leptonL.eta << std::endl;
      // std::cout << "d0sig = " << cand.leptonD0SignificanceL_PVrefit_includingPVError << std::endl;
      // std::cout << "relative iso = " << leptonL.iso/leptonL.pt << std::endl;
      // std::cout << "minValidCha DT = " << leptonL.dtStationsWithValidHits << std::endl;
      // std::cout << "minValidCha CSC = " << leptonL.cscStationsWithValidHits << std::endl;
      // 
      // std::cout << "High" << std::endl;
      // std::cout << "pt = " << leptonH.pt << std::endl;
      // std::cout << "eta = " << leptonH.eta << std::endl;
      // std::cout << "d0sig = " << cand.leptonD0SignificanceH_PVrefit_includingPVError << std::endl;
      // std::cout << "relative iso = " << leptonH.iso/leptonH.pt << std::endl;
      // std::cout << "minValidCha DT = " << leptonH.dtStationsWithValidHits << std::endl;
      // std::cout << "minValidCha CSC = " << leptonH.cscStationsWithValidHits << std::endl;
      // 	
      // std::cout << "cand" << std::endl;
      // // std::cout << "opposite charge: " << leptonL_.charge*leptonH_.charge << std::endl;
      // std::cout << "vertexChi2 = " << cand.vertexChi2 << std::endl;
      // std::cout << "deltaR = " << cand.deltaR << std::endl;
      // std::cout << "cosine = " << cand.cosine << std::endl;
      // std::cout << "mass = " << cand.corrDileptonMass << std::endl;
      // std::cout << "Lxysig = " << cand.decayLengthSignificance_PV << std::endl;
      // 
  ...
  // Control region
  fillPlots( cand, leptonL, leptonH, mass, dileptonCuts_controlColl_, controlCollCuts, h.histMapControlColl, false, false );
  fillPlots( cand, leptonL, leptonH, mass, dileptonCuts_looseControlColl_, looseControlCollCuts, h.histMapLooseControlColl, false );
...
bool DileptonAnalyzer::isComplementary(const TreeLepton & leptonL, const TreeLepton & leptonH, Histograms & h)
{
  ...
  for( unsigned int iTreeCand_=0; iTreeCand_ != candidates->candidates_.size(); ++iTreeCand_ ) {
    ...
    if(leptonL_.isCentralTrack && leptonH_.isCentralTrack) {
      DileptonAnalysisCuts::PassedWhichCuts finalCollCuts_muTrack = dileptonCuts_finalColl_muTrack.whichCuts( candidates->leptons_, cand_, cand_.corrDileptonMass, leptonL_,
    ...
    if (mindeltaRL < 0.1 && mindeltaRH < 0.1) {
      double min_D0Sig;
      if (dileptonCuts_finalColl_muTrack.get_use_PV()) {
	min_D0Sig = min( fabs(cand_.leptonD0SignificanceL_PV_includingPVError), fabs(cand_.leptonD0SignificanceH_PV_includingPVError) );
      }
      else {
	min_D0Sig = min( fabs(cand_.leptonD0SignificanceL_BS), fabs(cand_.leptonD0SignificanceH_BS) );
      }
      h.h_minLepton_d0Sig_mutrack_finalColl->Fill(min_D0Sig, eventWeight_);  
      return false;
    }
  }
