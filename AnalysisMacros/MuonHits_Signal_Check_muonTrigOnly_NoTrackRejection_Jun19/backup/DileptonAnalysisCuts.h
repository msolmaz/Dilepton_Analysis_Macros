...
class DileptonAnalysisCuts {
  public:
    ...
    PassedWhichCuts whichCuts( const std::vector<TreeLepton> & leptons_, TreeDipseudoLeptonCandidate & candidate, double mass, TreeLepton & leptonL,
  ...
  private:
    ...
    float cosinecut_cosmics; // Cosmics rejection
  
    bool rejectCosmics( const std::vector<TreeLepton> & leptons_, const TreeLepton & leptonL, const TreeLepton & leptonH, const float & cosineCut);
    bool rejectCosmic( const TreeLepton * l1, const TreeLepton * l2, const float & cosineCut );
