...
TString validStations="DT+CSC stations with valid hits";
...
//   histoSignal2->Scale(1/nInitialEventsSignal2);
  ...
  if (plotData) {
    ...
    TH1F * histoData1 = (TH1F*)inputFile->Get(tempName);
    outputFile->cd();
    histoData = (TH1F*) histoData1->Clone("histoData");
    inputFile->Close();
...
//  totalBkgMCIntegral = addBackgroundHistos(anaType, histoName, stack, legend, lumi, histoData->FindBin(3),histoData->GetNbinsX());
  ...
  //  if ( histoName=="nMinus1_vertexChi2_looseCutsA" || histoName=="nMinus1_vertexChi2_looseCutsB" || histoName=="nMinus1_vertexChi2_looseCutsC" ||
//         histoName=="nMinus1_vertexChi2_removedLifetimeCuts" ) {
//    cout << "DATA" << endl;
//    std::cout << "Integral of data between " << histoSignal1->GetBinLowEdge(1) << " and " << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 5 ))
//              << " : " << histoData->Integral(histoSignal1->GetBinLowEdge(1),histoSignal1->FindBin( 5 )) << std::endl;
//    std::cout << "Integral of data between " << histoSignal1->GetBinLowEdge(1) << " and " << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 8 ))
//              << " : " << histoData->Integral(histoSignal1->GetBinLowEdge(1),histoSignal1->FindBin( 8 )) << std::endl;
//    std::cout << "Integral of data between " << histoSignal1->GetBinLowEdge(1) << " and " << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 10 ))
//              << " : " << histoData->Integral(histoSignal1->GetBinLowEdge(1),histoSignal1->FindBin( 10 )) << std::endl;
//
//    std::cout << "Integral of background between " << histoSignal1->GetBinLowEdge(minIntegral) << " and " << histoSignal1->GetBinLowEdge(maxIntegral)
//              << " : " << totalBkgMCIntegral << std::endl;
//    if ( histoName=="nMinus1_vertexChi2_removedLifetimeCuts" ) {
//      cout << "Signal 1" << endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal1->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 5 )) << " : "
//                << histoSignal1->Integral( minIntegral, histoSignal1->FindBin( 5 ) ) / histoSignal1->Integral(minIntegral,histoSignal1->GetNbinsX() ) * 100
//                << std::endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal1->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 8 )) << " : "
//                << histoSignal1->Integral( minIntegral, histoSignal1->FindBin( 8 ) ) / histoSignal1->Integral(minIntegral,histoSignal1->GetNbinsX() ) * 100
//                << std::endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal1->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal1->GetBinLowEdge(histoSignal1->FindBin( 10 )) << " : "
//                << histoSignal1->Integral( minIntegral, histoSignal1->FindBin( 10 ) ) / histoSignal1->Integral(minIntegral,histoSignal1->GetNbinsX() ) * 100
//                << std::endl;
//      cout << "Signal 2" << endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal2->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal2->GetBinLowEdge(histoSignal2->FindBin( 5 )) << " : "
//                << histoSignal2->Integral( minIntegral, histoSignal2->FindBin( 5 ) ) / histoSignal2->Integral(minIntegral,histoSignal2->GetNbinsX() ) * 100
//                << std::endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal2->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal2->GetBinLowEdge(histoSignal2->FindBin( 8 )) << " : "
//                << histoSignal2->Integral( minIntegral, histoSignal2->FindBin( 8 ) ) / histoSignal2->Integral(minIntegral,histoSignal2->GetNbinsX() ) * 100
//                << std::endl;
//      std::cout << "Efficiency of signal for cut between " << histoSignal2->GetBinLowEdge(minIntegral) << " and "
//                << histoSignal2->GetBinLowEdge(histoSignal2->FindBin( 10 )) << " : "
//                << histoSignal2->Integral( minIntegral, histoSignal2->FindBin( 10 ) ) / histoSignal2->Integral(minIntegral,histoSignal2->GetNbinsX() ) * 100
//                << std::endl;
//    }
//  }
...
//   legendSignalData->AddEntry((TObject*)0,signal2Legend_part2,"");
  ...
  if( debug ) std::cout << "drawing mc/data ratio" << std::endl;

  // Draw mc/data ratio
  TGraphErrors ratioHist;
  if ( plotData && drawRatioPlot && totalBkgMCIntegral>0 ) {
    dp->cd();
  ...
  if( debug ) std::cout << "Save some histos as root files" << std::endl;
...
//      histoName=="minLeptonD0Sig_signWrtPerpDilepton_NoCorrections_removedLifetimeCuts" ||
//      histoName=="minLeptonD0Sig_signWrtPerpDilepton_d0CorrectedRunDependentNoVertex_removedLifetimeCuts" ||
...
void makeCommonPlots( TString anaType, TFile * outputFile, TString pdfOutputFile, double lumi, bool plotData, bool makeRatioPlot, TString cuts, bool plotCutValue ) {
  ...
  if ( plotData == false ) makeRatioPlot = false;
    ...
    makePlot("Lxy_"+cuts, outputFile, anaType, 0.6, 0.6, 0.9, 0.92,
        "Candidate L_{xy} [cm]", 0, 70, 0.01, 5000*yMaxFactor, true,
        false, -999,
        lumi, plotData, pdfOutputFile, makeRatioPlot );
...
//      lumi, plotData, pdfOutputFile );
  ...
  //  }
    // Muon only plots
    if ( anaType == "2muTrack" || anaType == "2saMu" ) {
...
//           lumi, plotData, pdfOutputFile, makeRatioPlot );
      ...
      makePlot("nMinus1_deltaR_"+cuts, outputFile, anaType, 0.5, 0.5, 0.85, 0.92,
	       "#DeltaR", 0, 1.0, 0.1, 500*yMaxFactor, true,
	       plotCutValue, 0.2,
	       lumi, plotData, pdfOutputFile, makeRatioPlot );
    }
  //
...
//         false, -999,
//         lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_minValidStations_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          validStations, 0, 20, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_minValidHits_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Valid Muon Hits", 0, 55, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 17,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_minValidDTHits_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Valid Muon DT Hits", 0, 55, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 17,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_minValidCSCHits_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Valid Muon CSC Hits", 0, 55, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 17,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_cosine_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "cosine", -1, 1, 0.1, 5000*yMaxFactor, true,
          plotCutValue, -0.75,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_DileptonAbsLxySig_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Absolute L_{xy} Significance", 0, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 12,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_AbsLeptonD0_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Absolute d_{0}/#sigma ", 0, 50, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 4,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_AbsD0_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Absolute d_{0} ", 0, 150, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 4,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_AbsDZ_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Absolute d_{Z} ", 0, 50, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 4,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    makePlot("nMinus1_AbsDZSignificance_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Minimum Absolute d_{Z}/#sigma ", 0, 50, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 4,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
    if ( anaType == "2muTrack" || anaType == "2saMu" ) {
      ...
      makePlot("nMinus1_maxTrackChi2_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Maximum track #chi^{2}/ndof", 0, 20, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 2,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeC_inverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeC_freeInverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Free Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeC_timeAtIpInOut_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time At IP In Out (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeC_timeAtIpOutIn_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time At IP Out In (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
/*      makePlot("nMinus1_muonTimeCSC_inverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeCSC_freeInverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Free Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeCSC_timeAtIpInOut_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time At IP In Out (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeCSC_timeAtIpOutIn_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time at IP Out In (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeDT_inverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeDT_freeInverseBeta_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Free Inverse Beta", -5, 5, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeDT_timeAtIpInOut_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time At IP In Out (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
      makePlot("nMinus1_muonTimeDT_timeAtIpOutIn_"+cuts, outputFile, anaType, 0.6, 0.5, 0.9, 0.93,
          "Time At IP Out In (ns)", -40, 40, 0.1, 5000*yMaxFactor, true,
          plotCutValue, 3,
          lumi, plotData, pdfOutputFile, makeRatioPlot );
*/
...
//           lumi, plotData, pdfOutputFile, makeRatioPlot );
    ...
    makePlot("nMinus1_absIso_"+cuts, outputFile, anaType, 0.35, 0.6, 0.8, 0.92,
        "#Sigma p_{T}", 0, 10, 0.1, 100000*yMaxFactor, true,
        false, 10,
        lumi, plotData, pdfOutputFile, makeRatioPlot );
...
//
  ...
  makePlot("cosmic_non_rejection_muonTimeC_inverseBeta_controlCollCuts", outputFile, anaType, 0.5, 0.4, 0.92, 0.92,
      "Lepton Inverse Beta", -5, 5, 0.01, 5000000, true,
      false, -999,
      lumi, true, pdfOutputFile, true );

  makePlot("cosmic_non_rejection_muonTimeC_freeInverseBeta_controlCollCuts", outputFile, anaType, 0.5, 0.4, 0.92, 0.92,
      "Lepton Free Inverse Beta", -5, 5, 0.01, 5000000, true,
      false, -999,
      lumi, true, pdfOutputFile, true );

  makePlot("cosmic_non_rejection_muonTimeC_timeAtIpOutIn_controlCollCuts", outputFile, anaType, 0.5, 0.4, 0.92, 0.92,
      "Lepton TimeAtIpOutIn (ns)", -40, 40, 0.01, 5000000, true,
      false, -999,
      lumi, true, pdfOutputFile, true );

  makePlot("cosmic_non_rejection_muonTimeC_timeAtIpInOut_controlCollCuts", outputFile, anaType, 0.5, 0.4, 0.92, 0.92,
      "Lepton TimeAtIpInOut (ns)", -40, 40, 0.01, 5000000, true,
      false, -999,
      lumi, true, pdfOutputFile, true );
