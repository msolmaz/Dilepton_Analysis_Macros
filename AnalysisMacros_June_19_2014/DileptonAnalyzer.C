...
void DileptonAnalyzer::studyTrackingEfficiency( const std::vector<TreeDipseudoLeptonCandidate> & cleanedCandidates, const std::vector<TreeLepton> & single_cosmic_muons )
{
...
void DileptonAnalyzer::studyCandidate( std::vector <TreeLepton> single_cosmic_muons, TreeDipseudoLeptonCandidate &cand, TreeLepton& leptonL, TreeLepton& leptonH , Histograms & h, bool fillCorrectedTextFiles )
{
...
*/
  ...
  cutsTemp = passedWhichCuts;
  cutsTemp.passAbsD0 = true;
  if (  cuts.passAllCuts( cutsTemp )  ) {
    histMap["AbsD0"]->Fill(min(fabs(leptonH.d0_PV), fabs(leptonL.d0_PV)), eventWeight_ );
  }

  cutsTemp = passedWhichCuts;
  cutsTemp.passAbsDZ = true;
  if (  cuts.passAllCuts( cutsTemp )  ) {
    histMap["AbsDZ"]->Fill(min(fabs(leptonH.dz_PV), fabs(leptonL.dz_PV)), eventWeight_ );
  }

  cutsTemp = passedWhichCuts;
  cutsTemp.passAbsDZSignificance = true;
  if (  cuts.passAllCuts( cutsTemp )  ) {
    histMap["AbsDZSig"]->Fill(min(fabs(leptonH.dzsignificance_PV), fabs(leptonL.dzsignificance_PV)), eventWeight_ );
  }
