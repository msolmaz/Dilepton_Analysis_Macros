...
minDeltaR_(-1), minCosine_(-9999), minDeltaPhi_(0), maxDeltaPhi_(99999999999), minMass_(0), min_csc_dt_station_valid_hits_(0), use_PV(true), cosinecut_cosmics(-999.0), track_reject_DR(9999.0) 
{
...
//   passedCuts.passRejectionCosmics=true;
  ...
  if( analysis == _2saMu ) {
    ...
    // This is redundant if the trackRejection is applied

    // if( !isComplementary(leptonL, leptonH, candidates, *dileptonCuts_finalColl_muTrack) ) passedCuts.passComplementarity = false;
    // if( !isComplementaryExtrapolated(candidate, candidates, *dileptonCuts_finalColl_muTrack ) ) passedCuts.passComplementarity = false;
