...
//DeltaR calculator
double DileptonAnalyzer::DeltaR(TreeLepton & lepton1, TreeLepton & lepton2){
...
//double deltaRL, deltaRH;
    for( unsigned int iTreeCand_=0; iTreeCand_ != candidates->candidates_.size(); ++iTreeCand_ ) {
      ...
      if(leptonL_.isCentralTrack && leptonH_.isCentralTrack){
        DileptonAnalysisCuts::PassedWhichCuts finalCollCuts_muTrack = dileptonCuts_finalColl_muTrack.whichCuts( cand_, cand_.corrDileptonMass, leptonL_, leptonH_, _2muTrack, lxyScale_, d0Scale_ );
//The following if loop is to make sure the muTrack cuts have been passed
        if (dileptonCuts_finalColl_muTrack.passAllCuts(finalCollCuts_muTrack)){ 
           double deltaRL = (DileptonAnalyzer::DeltaR(leptonL_,leptonL) < DileptonAnalyzer::DeltaR(leptonL_,leptonH)) ? DileptonAnalyzer::DeltaR(leptonL_,leptonL) : DileptonAnalyzer::DeltaR(leptonL_,leptonH);
           double deltaRH = (DileptonAnalyzer::DeltaR(leptonH_,leptonL) < DileptonAnalyzer::DeltaR(leptonH_,leptonH)) ? DileptonAnalyzer::DeltaR(leptonH_,leptonL) : DileptonAnalyzer::DeltaR(leptonH_,leptonH);
...
//To remove the overlap between track analysis and SA muon analysis...
 if (mindeltaRL < 0.1 && mindeltaRH < 0.1) return;
*/
  // Choose appropiate mass to plot
